...
variable "query_alert_rules" {
  ...
  description = <<EOT
    ...
    query_alert_rule = {
      ...
      evaluation_frequency              : "How often the alert rule is evaluated. Possible values are PT1M, PT5M, PT10M, PT15M, PT30M, PT45M, PT1H, PT2H, PT3H, PT4H, PT5H, PT6H, P1D."
      scopes                            : "The resource ID that the alert rule is scoped to. Currently, the API supports exactly 1 resource ID in the scopes list."
...
variable "activity_alert_rules" {
  type = list(object({
    ...
    criteria = optional(object({
      ...
      sub_statuses            = optional(set(string)),
